...
class SceneNet(Dataset):
    def __init__(self, root_path, protobuf_path, transform=None, target_transform=None):
        self.trajectories = sn.Trajectories()
        protobuf_path = os.path.join(root_path, protobuf_path)
    ...
    def __len__(self):
        return len(self.trajectories.trajectories)

    def __getitem__(self, idx):
        traj = self.trajectories.trajectories[idx]
        viewpoints = torch.zeros(len(traj.views), 6)
        images = torch.zeros(len(traj.views), 3, 64, 64)
        byte_to_tensor = lambda x: ToTensor()(x)
        for i, view in enumerate(traj.views):
            view_open = torch.tensor([view.shutter_open.camera.x, view.shutter_open.camera.y, view.shutter_open.camera.z, \
                                     view.shutter_open.lookat.x, view.shutter_open.lookat.y, view.shutter_open.lookat.z])
            view_close = torch.tensor([view.shutter_close.camera.x, view.shutter_close.camera.y, view.shutter_close.camera.z, \
                                     view.shutter_close.lookat.x, view.shutter_close.lookat.y, view.shutter_close.lookat.z])
            viewpoints[i] = (view_open + view_close) / 2.
            photo_path = photo_path_from_view(self.root_path, traj.render_path, view)
            images[i] = byte_to_tensor(Image.open(photo_path))
#             images[i] = byte_to_tensor(Image(photo_path))
